		...
		<div class="max-w-5xl mx-auto px-5 md:px-16 py-20 md:py-32">
			<div class="max-w-xl mx-auto text-center flex flex-col items-center gap-8" in:fly={{ y: 24, duration: 400 }}>
				...
				<p class="text-base text-on-surface-variant" style="line-height:1.6;">
					Zkontrolujte svůj e-mail a klikněte na potvrzovací odkaz do 30 minut — jinak příhoz vyprší.
				</p>
			</div>
		</div>
	</section>
{:else}
	<section class="bg-surface-container border-t border-outline-variant" id="prihodit">
		<div class="max-w-5xl mx-auto px-5 md:px-16 py-20 md:py-32">
			<div class="max-w-xl mx-auto text-center flex flex-col items-center gap-8">
				...
				<!-- Heading -->
				<div>
					<p class="text-xs font-semibold uppercase text-outline mb-3" style="letter-spacing: 0.1em;">Máte zájem?</p>
					<h2 class="font-display font-semibold text-on-surface text-3xl md:text-4xl" style="letter-spacing: -0.01em;">
						Formulář příhozu
					</h2>
				</div>

				<div class="max-w-xl mx-auto" in:fly={{ y: 32, duration: 400 }}>
					<form
						method="POST"
						action="/?/bid"
					...
					>
						...
						<fieldset class="rounded-2xl overflow-hidden" style="background:#fff; box-shadow:0 4px 24px rgba(108,72,32,0.08); border:1px solid #e9e8e5;">
							...
							<div class="px-6 pt-6 pb-4 border-b" style="border-color:#e9e8e5;">
								<label class="block text-xs font-semibold uppercase mb-2 text-outline" style="letter-spacing:0.08em;" for="name">
									Jméno a příjmení <span class="text-primary">*</span>
								</label>
								<input id="name" name="name" type="text" required autocomplete="name"
									bind:value={vals.name} placeholder="Jan Novák"
							...
							<div class="px-6 py-4 border-b" style="border-color:#e9e8e5;">
								...
								<input id="email" name="email" type="email" required autocomplete="email"
									bind:value={vals.email} placeholder="jan.novak@email.cz"
									class="w-full bg-transparent text-on-surface text-base outline-none placeholder:text-outline"
									style="border:none; padding:0; line-height:1.6;" />
							...
							<div class="px-6 pt-4 pb-6">
								<label class="block text-xs font-semibold uppercase mb-2 text-outline" style="letter-spacing:0.08em;" for="phone">
									Telefon <span class="text-primary">*</span>
								</label>
								<input id="phone" name="phone" type="tel" required autocomplete="tel"
									bind:value={vals.phone} placeholder="606 787 927"
									class="w-full bg-transparent text-on-surface text-base outline-none placeholder:text-outline"
									style="border:none; padding:0; line-height:1.6;" />
						...
						<fieldset>
							...
							<div class="flex items-center gap-2 mb-3">
								<p class="text-xs font-semibold uppercase text-outline" style="letter-spacing:0.08em;">Typ příhozu</p>
								<a href="/zpusob-nakupu" target="_blank" tabindex="-1"
									...
									title="Jak fungují typy příhozů?">?</a>
							</div>
							...
							<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
								...
								<label class="relative flex flex-col gap-3 rounded-2xl p-6 cursor-pointer transition-all duration-200"
									...
									<span class="text-lg font-semibold font-display text-on-surface">Jednorázový příhoz</span>
									<span class="text-xs text-on-surface-variant" style="line-height:1.6;">
										Přesná částka, kterou nabízíte. Zobrazí se ostatním okamžitě. Pokud vás někdo přehodí, musíte přihodit ručně.
									</span>
									<!-- Check indicator -->
									{#if !vals.isMax}
								...
								<label class="relative flex flex-col gap-3 rounded-2xl p-6 cursor-pointer transition-all duration-200"
									...
									<span class="text-lg font-semibold font-display" style="color:{vals.isMax ? '#fff' : '#1a1c1a'};">Maximální příhoz</span>
									<span class="text-xs" style="color:{vals.isMax ? '#ffe1d6' : '#54433c'}; line-height:1.6;">
										Systém za vás automaticky přehodí konkurenci — vždy jen o nejmenší nutný krok. Váš limit zůstane skrytý.
									</span>
									<!-- Check indicator -->
									{#if vals.isMax}
					...
					<div class="rounded-2xl px-6 py-5" style="background:#fff; box-shadow:0 4px 24px rgba(108,72,32,0.08); border:1px solid #e9e8e5;">
						...
						<label class="block text-xs font-semibold uppercase mb-3 text-outline" style="letter-spacing:0.08em;" for="price">
							{vals.isMax ? 'Maximální částka' : 'Výše příhozu'} <span class="text-primary">*</span>
						</label>
						<div class="flex items-baseline gap-2">
							<input id="price" name="price" type="text" inputmode="numeric" required
								value={vals.price} oninput={formatPrice} placeholder="2 500 000"
						...
						<label class="flex items-start gap-3 cursor-pointer">
							...
							<span class="text-sm text-on-surface-variant" style="line-height:1.6;">
								Souhlasím s
								<a href="/obchodni-podminky" target="_blank" class="text-primary underline underline-offset-2 hover:text-primary-container">obchodními podmínkami</a>
								výběrového řízení. <span class="text-primary">*</span>
							</span>
						</label>
